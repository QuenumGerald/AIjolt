...
export function parseDetail(detail: string, path: string, fallbackTitle: string, baseUrl: string): RawJob {
  ...
  return { externalId: path.split('/').filter(Boolean).at(-1) ?? path, title, company, location, description, postedAt, url: `${baseUrl}${path}`, source: 'foorilla' };
}
export async function foorilla(pages: number, baseUrl: string): Promise<RawJob[]> {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    for (const link of links) {
      ...
      jobs.push(parseDetail(detail, link.path, link.title, baseUrl));
    }
