...
export async function greenhouse(board: string, company = board): Promise<RawJob[]> {
  ...
  return data.jobs.map(j => ({ externalId: String(j.id), title: j.title, company, location: j.location?.name, description: j.content, postedAt: j.updated_at, url: j.absolute_url, source: 'greenhouse' }));
}
